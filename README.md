...
discovers the headset by the Android Head Tracker HID descriptor, which appears
as a `/dev/hidraw*` node. Validated on a WH-1000XM5, including Assetto Corsa
Competizione through Proton.

Install the desktop app:

```bash
# Fedora
sudo dnf copr enable melcodesdev/sony-head-tracker && sudo dnf install sony-head-tracker

# Arch, CachyOS, Manjaro: build the AUR package in packaging/aur/
makepkg -si

# Any distribution, from source (needs g++ 13+, GTK 4, libadwaita 1.4+)
make && sudo make install PREFIX=/usr
```

Then launch **Sony Head Tracker**, click **Grant device access** once (it installs
a udev rule so `/dev/hidraw*` is readable without root), reconnect the headset, and
press **Start**. **Set up a game** wires OpenTrack for a Steam or Proton title.
...
Per distribution packages, the global recenter shortcut, Proton game setup, and
troubleshooting are in [`docs/LINUX.md`](docs/LINUX.md). The pipeline internals
...
Needs a C++20 compiler (g++ 13+ or clang 16+) and Linux headers
(`linux/hidraw.h`). No external libraries.

```bash
make            # -> build/sony-head-tracker
make test       # Linux test suite
make portable   # static libstdc++, for redistributable binaries (Steam Deck)
```

CMake works too, via the `if(LINUX)` block:
...
The desktop app needs Python 3, PyGObject, GTK 4 and libadwaita 1.4 or newer. See
